...
### UTXO 模型
UTXO全名是Unspent Transaction Outputs，未花费交易输出，它的总体的设计是基于这样一种思路，如果A要花费一笔钱比如100元，这笔钱不会凭空产生，那么必然由B先花费了100元，被A赚到这100元，然后A才能继续花费这一笔钱。这个链条的源头就是铸币。然后会产生这样一个链条，铸币->B->A->?
...
比特币的UTXO模型整体上是这样设计的，但是交易的输入和输出却并不简单只有数值，甚至还包含一段脚本，而这段脚本就是用来验证谁有权来花费这些余额。
